# Stage 3k: partially-linear full model via XGBoost interaction_constraints.
# D is placed in its own singleton interaction group (verified by tree
# inspection at build time — no fitted tree combines a D-split with an
# X-split), so the ensemble decomposes exactly into f(X) + h(D), h(D) an
# additive offset since D is binary. One fit, no backfitting; deliberately
# not heavily tuned (max_depth=4, eta=0.1, nrounds=100) since the point is
# testing whether the structural constraint alone protects an under-tuned
# model, mirroring the untuned-nnet test in stage 3f/3j.
#
# Three sub-analyses, same pattern as 3d-3j:
...
# Run from repo root: Rscript R/stage3k_pl_xgboost.R
...
cat("== (a) bias/coverage MC: pl_xgboost (interaction_constraints), B=25, n=2000 ==\n\n")
est_imp <- function(d) sudo_binary(d, full_model = "pl_xgboost")
est_boot <- function(d) sudo_binary(d, full_model = "pl_xgboost",
                                    proper_boot = TRUE)
arms <- list(
  list(name = "improper_plxgb_t1.5", est = est_imp, theta = 1.5,
       n_reps = 100, seed = 8000),
  list(name = "improper_plxgb_t3", est = est_imp, theta = 3,
       n_reps = 100, seed = 2000),
  list(name = "boot_plxgb_t1.5", est = est_boot, theta = 1.5,
       n_reps = 100, seed = 8000),
  list(name = "boot_plxgb_t3", est = est_boot, theta = 3,
       n_reps = 100, seed = 2000))
all_summ <- list()
for (a in arms) {
  ...
  s <- summarize_mc(df)
  print_mc(a$name, s)
  all_summ[[length(all_summ) + 1]] <- cbind(stage = "3k", estimator = a$name, s)
}
cat("\n")
...
calib_one <- function(d, theta_true) {
  ...
  lp_hat <- fit_pl(d, folds, "xgboost")$lp_hat
  fit <- lm(d$eta ~ lp_hat)
...
for (th in c(1.5, 3)) {
  ...
  m <- rowMeans(res)
  cat(sprintf("theta=%.1f pl_xgboost  slope %.3f  intercept %+.3f  R2 %.3f\n",
              th, m["slope"], m["intercept"], m["r2"]))
  calib_rows[[length(calib_rows) + 1]] <- data.frame(
    stage = "3k", theta = th, learner = "pl_xgboost",
...
for (th in c(1.5, 3)) {
  ...
  cat(sprintf(
    "theta=%.1f pl_xgboost  mean var_ratio %.3f (t-test p=%.2e)  frac p_het<0.05: %.2f  mean|theta_bias| %.3f\n",
...
write.csv(sm, "R/results/stage3k_summary.csv", row.names = FALSE)
write.csv(calib_sm, "R/results/stage3k_calib.csv", row.names = FALSE)
write.csv(vr_sm, "R/results/stage3k_varratio.csv", row.names = FALSE)
cat("wrote R/results/stage3k_{summary,calib,varratio}.csv\n")
